-- جداول نظام الشغلانات / المشاريع — شغّل مرة واحدة على المشروع الجديد
...
DROP POLICY IF EXISTS "authenticated_all" ON project_revenues;
DROP POLICY IF EXISTS "authenticated_all" ON project_expenses;
DROP POLICY IF EXISTS "authenticated_all" ON project_custodies;
DROP POLICY IF EXISTS "authenticated_all" ON estimated_assets;
DROP POLICY IF EXISTS "authenticated_all" ON employee_deductions;
...
CREATE POLICY "authenticated_all" ON project_revenues
  FOR ALL TO authenticated
  USING (auth.role() = 'authenticated')
  WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "authenticated_all" ON project_expenses
  FOR ALL TO authenticated
  USING (auth.role() = 'authenticated')
  WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "authenticated_all" ON project_custodies
  FOR ALL TO authenticated
  USING (auth.role() = 'authenticated')
  WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "authenticated_all" ON estimated_assets
  FOR ALL TO authenticated
  USING (auth.role() = 'authenticated')
  WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "authenticated_all" ON employee_deductions
  FOR ALL TO authenticated
  USING (auth.role() = 'authenticated')
  WITH CHECK (auth.role() = 'authenticated');
